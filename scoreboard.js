 ...
 * editsportcode - sport code at new game
 * editperiod
 * newpenalty
 * editpenalty
 * editpenaltytime
...
{
	if (cleartimerid && (msg != false))
	{
		clearTimeout(cleartimerid);
		cleartimerid = 0;
	}
	document.getElementById('acceptdiv').style.visibility = 'hidden';
	if ((msg === false) && (lcdbuffer == "") ) {
		var scrntext = "<div style='display:inline-block;width:49%;text-align:left;'>Home: " + homescore + "</div>";
		scrntext += "<div style='display:inline-block;width:49%;text-align:right;'>Away: " + awayscore + "</div>";
		scrntext += "<div style='width:100%;text-align:center;'>";
		if (toddisplay) {
		} else {
			if (mainmin < 10) scrntext += "0";
			scrntext += mainmin + ":";
			if (mainsec < 10) scrntext += "0";
			scrntext += mainsec;
		}
		if (autohorn) scrntext += " h";
		scrntext += "</div>";
		document.getElementById('scrn').innerHTML = scrntext;
	} else {
		if (msg !== false) {
			lcdbuffer = msg;
